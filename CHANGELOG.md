...
- Added `trailingNewline` (default `true`) to `FormatMarkdownOptions`: set it to `false` to strip every trailing newline and add none back, instead of forcing exactly one. For formatting a FRAGMENT that will be embedded inside a larger document, where the usual whole-file "exactly one final newline" guarantee would be unwanted.
- Added `ignoreGridTables` (default `false`) to `FormatMarkdownOptions`: set it to `true` to never recognize a Pandoc/reST-style grid table (`+---+` borders) as a table at all — a matching block passes through byte-identical instead of being parsed and re-emitted as a pipe table. For formatting a FRAGMENT of a larger document, where unrelated content (e.g. another tool's own `+---+`-bordered output) can coincidentally satisfy the grid-table border grammar without being one.
- Added `reflow` (default `true`) to `FormatMarkdownOptions`: set it to `false` to skip paragraph/list-item reflow and list-tightening entirely, leaving every non-table line exactly as authored. This is what makes `formatMarkdown` agree with `minifyMarkdown` on prose — `minifyMarkdown` never reflows, so previously there was no way to get `formatMarkdown`'s table handling without also getting its paragraph-joining.
- **BREAKING: renamed `tablesToLogicalRows` → `minifyMarkdown`.** Same function (reduce every table in a document to its minimal logical form, everything else byte-identical) — `minifyMarkdown` says what it does at a glance; note it still touches ONLY tables (no paragraph/list reflow), unlike `formatMarkdown`.
- **BREAKING: renamed `emitLogicalTable` → `minifyTable`.** Same function (emit one table in the minimal, no-column-padding serializer form) — the old name read as if it produced logical *rows* like `minifyMarkdown` does, when it actually emits one already-parsed table; `minifyTable` says what it does instead.
- Fixed the pipe-table engine silently merging a body row with a blank first cell into the row above it (data loss: the row's content got appended into the previous row's last cell and the row itself vanished from the table). This heuristic (`collapseContinuationRows`) existed only to undo this formatter's own now-retired continuation-row wrapping at nonzero `tableWidth` (see 0.7.0's grid tables) — nonzero-width wrapping no longer produces that shape at all, so every pipe table is now assumed to already be properly authored GFM, with the sole special case still being GFM's own whole-row-spanning-note convention (a body row with exactly one cell). See docs/design.md's "Sparse rows" section.
